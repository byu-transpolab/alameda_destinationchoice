...
d_ratio_base <- (-1 * coefficients(base_models$`Network Distance`)[1] / 
  coefficients(base_models$`Network Distance`)[2] )^( -1)
d_ratio_sport <- ( -1 * coefficients(base_models$`Sport Detail`)[1] / 
  coefficients(base_models$`Sport Detail`)[2]  )^( -1)
```
...
that on average, people will travel  `r d_ratio_base` times further to reach a park
twice as large.
...
the implied distance to size trade-off changing to `r d_ratio_sport`. Examining
...
[@benakiva, p. 300]. In this application, these benefits are apparently small, on the order
