...
acs <- get_acs(geography = "block group", variables = variables, year = 2017,
  ...
  transmute(
    ...
    other_race   = 100 * (nativeam + pacific) / population,
    white_hisp   = 100 - black - asian - other_race
  ) %>%
  filter(population > 0)
...
acs_for_table <- acs %>% select(
    ...
    "Other: Share of population who belong to other minority groups" = other_race)

if(knitr::is_latex_output()){ 
  datasummary_skim(acs_for_table, title = "Block Group Summary Statistics",
    histogram = FALSE, output = "latex") %>%
...
logitdata <- mydata %>%
  ...
  left_join(acs %>% select(geoid, density:other_race) %>% st_set_geometry(NULL),
            by = c("home" = "geoid")
