...
We identified trails and footpaths using the `path`, `cycleway`, and
...
that surrounds them [@clifton2016]. Accordingly, we counted the number of features
in OpenStreetMap that are tagged with the amenities `bar`, `cafe`, `fast_food`, 
`restaurant`, `bank`, and `pharmacy` within 500 feet of the boundary of each park. 
...
datasummary_balance(
  ...
  data = attributed_parks %>% 
    transmute(Access = access, Acres = acres, 
           ...
           Trail = trail, 
           `Mobile Devices` =attractions, `Shops` = shops), 
...
person $n$ [@Williams1977a, @benakiva],
\begin{equation}
...
monetary terms [@Handy1997;@Dong2006]. Note also that the logsum increases
...
any value of $V_q$ [@benakiva, p. 300]. 
...
coefficients but the coefficients remain unbiased [@train2009]; a more elegant 
...
"Travel Model One" activity-based travel demand model [@mtctm1]. In this model,
